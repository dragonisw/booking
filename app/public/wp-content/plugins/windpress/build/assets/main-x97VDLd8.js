...
`).querySelector(`#windpressbricks-plc-input`), x = [], S = document.createRange().createContextualFragment(`
...
`).querySelector(`#windpressbricks-plc-class-sort`), C = document.createRange().createContextualFragment(`
...
`).querySelector(`#windpressbricks-plc-class-down`);
  x.push(S), x.push(C);
  var w = r(false), T = r([]), E = r(0), D = null;
  d(b);
  var O = [];
  ...
  k.events.callbacks = function() {
    ...
    g.$_state.selectedElements.length > 0 ? T.value = g.$_state.selectedElements.map((e3) => e3.id) : g.$_activeElement.value ? T.value = [
      g.$_activeElement.value.id
    ] : T.value = [];
  }).observe(document.querySelector(`#bricks-structure`), {
    subtree: true,
    attributes: true,
    childList: true
  }), new MutationObserver(function(e2) {
    n(() => {
      E.value === g.$_state.historyIndex && (E.value = 0), E.value = g.$_state.historyIndex;
    });
  }).observe(document.querySelector(`#bricks-toolbar > ul.group-wrapper.end > li.undo`), {
    subtree: false,
    attributeFilter: [
      `class`
    ]
    ...
    (e2[0] !== t2[0] || e2[2] !== t2[2]) && n(() => {
      if (e2[0].length > 0) {
        let t3 = e2[0].map((e3) => g.$_state.activeComponent ? g.$_state.components.find((e4) => e4.id === g.$_state.activeComponent.id)?.elements.find((t4) => t4.id === e3)?.settings?._cssClasses || `` : g.$_state[z()].find((t4) => t4.id === e3)?.settings?._cssClasses || ``);
        b.value = t3.every((e3) => e3 === t3[0]) ? t3[0] : ``;
      } else b.value = ``;
      j();
    }), e2[0] && e2[1] && n(() => {
      let e3 = document.querySelector(`#bricks-panel-sticky`);
      if (m(`module.plain-classes.input-field`, true).value && e3) {
        if (e3.querySelector(`.windpressbricks-plc-input`) === null) {
          let t4 = document.createElement(`div`);
          t4.style.padding = `0 var(--builder-spacing)`, t4.appendChild(b), e3.appendChild(t4), D = new f(b, {
            highlight: [
              {
                highlight: /(?<=\s|^)(?:(?!\s).)+(?=\s|$)/g,
                className: `word`
        ...
        let t3 = document.querySelector(`#bricks-panel-element-classes > div > div.actions-wrapper > div > div.dropdown-wrapper > div`);
        t3.querySelectorAll(`.windpressbricks-plc`).length === 0 && x.forEach((e4) => {
          t3.appendChild(e4);
        });
      }
    });
    ...
    T.value.forEach((t2) => {
      if (g.$_state.activeComponent) {
        let n2 = g.$_state.components.find((e3) => e3.id === g.$_state.activeComponent.id);
        if (n2) {
          let r2 = n2.elements.find((e3) => e3.id === t2);
          r2 && (r2.settings._cssClasses = e2.target.value);
        }
      } else {
        let n2 = g.$_state[z()].find((e3) => e3.id === t2);
  ...
  b.addEventListener(`tribute-active-true`, function(e2) {
    ...
    b.value = await _.contentWindow.windpress.module.classSorter.sort(b.value);
    let t2 = new Event(`input`, {
      bubbles: true
    });
    b.dispatchEvent(t2), j();
    ...
    let r2 = new Event(`input`, {
      bubbles: true
    });
    b.dispatchEvent(r2), t2.settings._cssGlobalClasses = [], j();
  });
  function I(e2) {
  ...
  function z() {
    let e2 = g.$_state.templateType;
    return [
      `header`,
      `footer`
    ].includes(e2) ? e2 : `content`;
  }
  o(`Module loaded!`, {
