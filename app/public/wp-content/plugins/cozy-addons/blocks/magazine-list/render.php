...
wp_localize_script( 'cozy-block--magazine-list--frontend-script', $block_id, $attributes );
wp_add_inline_script( 'cozy-block--magazine-list--frontend-script', 'document.addEventListener("DOMContentLoaded", function(event) { window.cozyBlockMagazineList( "' . esc_html( $client_id ) . '" ) }) ' );
...
#$block_id .post__title {
	...
	font-size: 	clamp(12px, 1.185rem + ((1vw - 3.2px) * 0.673), {$attributes['postOptions']['title']['font']['size']});
...
if ( ! function_exists( 'render_cozy_block_magazine_list_posts_data' ) ) {
	function render_cozy_block_magazine_list_posts_data( $attributes, $post_data, &$output ) {
		...
		if ( $attributes['enableOptions']['postImage'] && ! empty( $post_data['post_image_url'] ) && 'left' === $attributes['postOptions']['image']['position'] ) {
			$classes   = array();
			$classes[] = 'post__image';
		...
		if ( $attributes['enableOptions']['postImage'] && ! empty( $post_data['post_image_url'] ) && 'right' === $attributes['postOptions']['image']['position'] ) {
			$classes   = array();
			$classes[] = 'post__image';
			$classes[] = $attributes['postOptions']['image']['hoverEffect'] ? 'has-hover-effect' : '';
