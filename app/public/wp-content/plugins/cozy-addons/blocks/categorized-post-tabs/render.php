...
#$block_id .cozy-block-categorized-post-tabs__heading {
    ...
    {$heading['border']}
    {$heading['radius']}
    font-size: clamp(12px, calc(3vw + 4px), {$attributes['headingStyles']['font']['size']});
...
#$block_id .cozy-block-categorized-post-tabs__tab {
    ...
    font-size: clamp(12px, calc(3vw + 4px), {$attributes['tabStyles']['font']['size']});
...
#$block_id .featured-post__title {
	...
	font-size: clamp(10px, calc(3vw + 4px), {$attributes['featuredPostOptions']['title']['font']['size']});
...
#$block_id .post__title {
	...
	font-size: clamp(10px, calc(3vw + 4px), {$attributes['postOptions']['title']['font']['size']});
		...
		if ( ! empty( $post_data ) ) {
			...
			if ( isset( $attributes['enableOptions']['featuredPostContent'] ) && $attributes['enableOptions']['featuredPostContent'] ) {
				...
				if ( isset( $post_data['post_excerpt'] ) && ! empty( $post_data['post_excerpt'] ) ) {
					$output .= cozy_create_excerpt( $post_data['post_excerpt'], $featured_excerpt );
				} else {
